...
def extract_customers(engine: Engine) -> list[Customer]:
...
def extract_orders(engine: Engine) -> list[Order]:
    with engine.connect() as conn:
...
def extract_currency_exchanges(engine: Engine) -> list[CurrencyExchange]:
    with engine.connect() as conn:
